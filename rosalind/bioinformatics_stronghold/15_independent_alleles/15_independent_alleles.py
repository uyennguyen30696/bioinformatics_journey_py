...
# Using Punnett square, the probability of the offsprng to be Aa Bb is 0.25
# The probability of the offspring to NOT be Aa Bb is 0.75
...
# P = total events = total population after k-th generations = 2^k
p = pow(2, k)

# i = number of times for a specific outcome within n trials (number of times for N = Aa Bb within k-th generations)
# i = n 

def nCr(n, r):
    probability = (math.factorial(n) / ((math.factorial(n - r) * math.factorial(r))))
    return probability

def prob(N, p):
    result = 0
    for i in range(N, p + 1):
        prob = nCr(p, i) * pow(0.25, i) * pow(0.75, p - i)
        result += prob
